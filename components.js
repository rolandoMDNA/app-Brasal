...
export const NavBar = () => `
    <nav class="glass flex items-center justify-between px-4 py-3 shadow-sm border-b border-slate-100">
        <button onclick="goBack()" class="p-3 -ml-2 rounded-full hover:bg-slate-100 text-slate-600 flex items-center gap-1 font-medium">
            ${Icons.Back}
            <span class="text-sm">Atrás</span>
        </button>
        <button onclick="navigate('home')" class="p-3 -mr-2 rounded-full hover:bg-slate-100 text-slate-600 flex items-center gap-2 font-medium">
...
export const OrdersHistory = (groupedOrders) => {
                                ...
                                ${Object.keys(dates[date]).map(clientName => `
                                    <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                                        <div class="mb-3 border-b border-slate-50 pb-2">
                                            <p class="font-bold text-slate-800 text-lg">${clientName}</p>
                                        </div>
                                        <div class="space-y-2">
                                            ${dates[date][clientName].map(item => `
                                                <div class="flex justify-between items-center bg-slate-50 px-3 py-2 rounded-lg">
                                                    <span class="text-sm font-medium text-slate-600">${item.product}</span>
                                                    <span class="font-bold text-slate-800">${item.value} uds</span>
                                                </div>
                                            `).join('')}
                                        </div>
                                    </div>
    ...
    return `
        <div class="mb-6">
            <h2 class="text-2xl font-bold text-slate-800">Pedidos Registrados</h2>
            <p class="text-slate-500">Agrupados por cliente y fecha de entrega</p>
        </div>
        <div class="pb-24">
...
export const DailyReports = (history) => {
    ...
    return `
        <div class="mb-6">
            <h2 class="text-2xl font-bold text-slate-800">Informes Diarios</h2>
            <p class="text-slate-500">Historial por fechas y exportación</p>
        </div>
        ...
        <div class="space-y-3 pb-20">
            ...
            uniqueDates.map(date => {
                ...
                return `
                    <button onclick="navigate('report-detail', { date: '${date}' })" class="w-full bg-white border border-slate-200 p-5 rounded-2xl shadow-sm text-left active:bg-slate-50">
                        <div class="flex justify-between items-center mb-3">
                            <span class="font-bold text-slate-800 text-lg">${date.split('-').reverse().join('/')}</span>
...
export const RouteMenu = (route) => `
    <div class="mb-8">
        <h2 class="text-2xl font-bold text-slate-800">Ruta: ${route.name}</h2>
        <p class="text-slate-500">Selecciona una acción</p>
    </div>
...
export const ClientList = (clients, typeLabel, routeName, originalType, routeId) => `
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-slate-800 capitalize">${typeLabel}</h2>
        <p class="text-slate-500">Cliente en Ruta ${routeName}</p>
    ...
    <div class="grid grid-cols-1 gap-2 pb-10" id="client-list-container">
        ${clients.map(client => `
            <div data-client="${client}" class="bg-white border border-slate-200 rounded-2xl shadow-sm text-left active:bg-slate-50 transition-colors flex items-center justify-between overflow-hidden">
                <div class="flex items-center gap-3 p-4 flex-1" onclick="selectClient('${client}', '${originalType}', ${routeId})">
                    ...
                    <span class="text-lg font-bold text-slate-700 truncate select-none">${client}</span>
                </div>
                <button onclick="selectClient('${client}', '${originalType}', ${routeId})" class="p-4 bg-slate-50 border-l border-slate-100 hover:bg-slate-100 transition-colors h-full">
    ...
    <div class="flex flex-col h-full">
        <div class="text-center mb-6">
            ...
            <div class="mt-4 p-6 bg-slate-100 rounded-2xl">
                ...
                <span class="text-2xl text-slate-500 ml-2">${unit}</span>
            </div>
        </div>
        
        <div class="grid grid-cols-3 gap-3 flex-1 pb-10">
            ${[1, 2, 3, 4, 5, 6, 7, 8, 9].map(n => `<button onclick="numpadPress('${n}')" class="h-20 text-3xl font-bold bg-white border border-slate-200 rounded-2xl shadow-sm active:bg-slate-50">${n}</button>`).join('')}
            ${type === 'payments' ? `<button onclick="numpadPress('.')" class="h-20 text-3xl font-bold bg-white border border-slate-200 rounded-2xl shadow-sm active:bg-slate-50">.</button>`
        : `<button onclick="numpadPress('.5')" class="h-20 text-2xl font-bold bg-blue-50 text-blue-600 border border-blue-200 rounded-2xl shadow-sm active:bg-blue-100">+ &frac12;</button>`}
            <button onclick="numpadPress('0')" class="h-20 text-3xl font-bold bg-white border border-slate-200 rounded-2xl shadow-sm active:bg-slate-50">0</button>
...
export const AdminMenu = () => `
    <div class="mb-8">
        <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Menú de Gestión</h2>
        <p class="text-slate-500 font-medium">Administración de clientes y rutas</p>
    </div>
    <div class="grid grid-cols-1 gap-4">
        <button onclick="navigate('add-client-name')" class="btn-touch bg-white border-2 border-slate-200 text-slate-800 shadow-sm">
            <i data-lucide="user-plus" class="w-6 h-6 text-blue-500"></i>
            <span class="font-bold">Añadir Nuevo Cliente</span>
        </button>
        <button onclick="navigate('del-client-route')" class="btn-touch bg-white border-2 border-slate-200 text-red-500 shadow-sm">
            <i data-lucide="user-minus" class="w-6 h-6 text-red-500"></i>
            <span class="font-bold">Eliminar Cliente Existente</span>
        </button>
        <button onclick="navigate('past-returns-date')" class="btn-touch bg-white border-2 border-teal-200 text-teal-700 shadow-sm mt-4">
...
export const PastReturnsDate = () => `
    <div class="mb-8">
        <h2 class="text-2xl font-bold text-slate-800">Fecha Pasada</h2>
        <p class="text-slate-500">Selecciona el día que quieres registrar (ej: devoluciones de ayer)</p>
    </div>
    <div class="space-y-6">
        <input type="date" id="past-returns-date-input" 
               class="w-full h-16 px-6 text-xl border-2 border-slate-200 rounded-2xl focus:border-teal-500 outline-none">
...
export const PastReturnsRoute = (customDate) => `
    <div class="mb-8">
        <h2 class="text-2xl font-bold text-slate-800">Asignar Ruta</h2>
        <p class="text-slate-500">¿Para qué ruta registrarás datos el ${customDate.split('-').reverse().join('/')}?</p>
    </div>
    <div class="grid grid-cols-1 gap-4">
...
export const PastRecordsType = (routeId, customDate) => `
    <div class="mb-8">
        <h2 class="text-2xl font-bold text-slate-800">Tipo de Registro</h2>
        <p class="text-slate-500">¿Qué deseas registrar para el ${customDate.split('-').reverse().join('/')}?</p>
    </div>
    <div class="grid grid-cols-1 gap-4">
...
export const AddClientName = () => `
    <div class="mb-8">
        <h2 class="text-2xl font-bold text-slate-800">Nombre del Cliente</h2>
        <p class="text-slate-500">Ingresa el nombre comercial</p>
    </div>
    <div class="space-y-6">
        <input type="text" id="new-client-name" placeholder="Ej: Panadería El Sol" 
               class="w-full h-16 px-6 text-xl border-2 border-slate-200 rounded-2xl focus:border-blue-500 outline-none">
...
export const AddClientRoute = () => `
    <div class="mb-8">
        <h2 class="text-2xl font-bold text-slate-800">Asignar Ruta</h2>
        <p class="text-slate-500">¿A qué zona pertenece este cliente?</p>
    </div>
    <div class="grid grid-cols-1 gap-4">
...
export const AddClientLists = () => `
    <div class="mb-8 text-center">
        <h2 class="text-2xl font-bold text-slate-800">Listas del Cliente</h2>
        <p class="text-slate-500">¿En qué apartados debe aparecer?</p>
    </div>
    <div class="grid grid-cols-1 gap-3">
        <button onclick="finalizeNewClient('pedidos')" class="btn-touch bg-white border border-slate-200 text-slate-700">
            ${Icons.Orders}
            <span>LISTA DE PEDIDOS</span>
        </button>
        <button onclick="finalizeNewClient('devoluciones')" class="btn-touch bg-white border border-slate-200 text-slate-700">
            ${Icons.Returns}
            <span>LISTA DE DEVOLUCIONES</span>
        </button>
        <button onclick="finalizeNewClient('cobros')" class="btn-touch bg-white border border-slate-200 text-slate-700">
            ${Icons.Payments}
...
export const ReportDetail = (date, groupedEntries) => {
    ...
    return `
        <div class="mb-6">
            <h2 class="text-2xl font-bold text-slate-800">Informe: ${date.split('-').reverse().join('/')}</h2>
            <p class="text-slate-500">Registros agrupados por cliente</p>
        </div>
        ...
        <div class="space-y-6 pb-24">
            ...
            clientNames.map(client => `
                <div class="bg-white border-2 border-slate-100 rounded-3xl p-5 shadow-sm">
                    <h4 class="font-bold text-slate-900 text-lg mb-4 border-b border-slate-50 pb-2">${client}</h4>
                    <div class="space-y-3">
                        ${groupedEntries[client].map(entry => `
                            <div class="flex justify-between items-center">
                                <div>
                                    ...
                                    <p class="text-xs font-bold text-slate-500">${entry.product || (entry.type === 'payments' ? 'Cobro en efectivo' : '')}</p>
                                </div>
                                <div class="text-right flex items-center gap-3">
                                    <p class="font-black text-slate-800">${entry.value}${entry.type === 'payments' ? '€' : ' uds'}</p>
...
export const DeleteClientRoute = () => `
    <div class="mb-8">
        <h2 class="text-2xl font-bold text-slate-800">Eliminar Cliente</h2>
        <p class="text-slate-500">¿De qué ruta deseas eliminar?</p>
    </div>
    <div class="grid grid-cols-1 gap-4">
...
export const DeleteClientSelection = (routeId, clients) => `
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-slate-800">Seleccionar Cliente</h2>
        <p class="text-slate-500">Ruta ${routeId === 1 ? 'Rubén' : 'José'}</p>
    </div>
    <div class="grid grid-cols-1 gap-2 pb-10">
        ${clients.map(client => `
            <button onclick="finalizeDeleteClient(${routeId}, '${client}')" 
                    class="flex items-center justify-between p-5 bg-white border border-red-100 rounded-xl hover:border-red-400 text-left cursor-pointer">
                <span class="text-lg font-medium text-slate-700">${client}</span>
